...
import {
  Calendar, Plus, Trash2, CheckCircle, Circle, Pencil, RefreshCw, ClipboardList, Layers, ChevronLeft, ChevronRight,
} from "lucide-react";
...
export default function Scheduler() {
  ...
  // Pagination Distribusi Agenda (4 card per halaman)
  ...
  const handleUpdateStatus = async (SchedulerId, currentStatus) => {
    ...
    try {
      // Backend atomik: update status scheduler & insert data aktivitas baru
      ...
      // Setelah status diubah menjadi Done, refresh data aktivitas murni dari database
      if (nextStatus === "Done") {
        const freshAktivitas = await GetAktivitas();
        setAktivitasList(Array.isArray(freshAktivitas) ? freshAktivitas : []);
      }
    } catch (error) {
  ...
  // Pagination & Sort Blok Penanaman
  const penanamanIds = Object.keys(groupedSchedulers).sort((a, b) => {
    const hasTodayA = groupedSchedulers[a].some((s) => {
      ...
      return dateStr === todayStr && s.Status !== "Done";
    });
  ...
  // Riwayat Log murni dari Database (aktivitasList)
  const dbLogs = aktivitasList
    .map((a) => ({
      id: `aktivitas-${a.AktivitasId}`,
      jenis: a.JenisAktivitas,
      tanggal: a.Tanggal ? a.Tanggal.split("T")[0] : "",
      keterangan: a.Keterangan || "-",
      isFromScheduler: Boolean(a.SchedulerId && a.SchedulerId > 0)
    }))
  ...
  return loading ? (
    ...
    <div className="scheduler animate-fade-in p-6 max-w-7xl mx-auto space-y-6">
        ...
        <form className="flex flex-wrap md:flex-nowrap items-end gap-3" onSubmit={handleCreateOrUpdateScheduler}>
          ...
          <div className="flex-1 min-w-[120px] flex flex-col">
            ...
            >
              ...
              <option value="Done" className="bg-white dark:bg-slate-800 text-gray-700 dark:text-slate-100">Done</option>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 max-w-7xl items-start">
        ...
        {/* KOLOM KIRI (Distribusi Agenda per Blok Penanaman) */}
              ...
              {currentPenanamanIds.length === 0 ? (
                ...
                currentPenanamanIds.map((penanamanId) => {
                  const sortedAgenda = [...groupedSchedulers[penanamanId]].sort((a, b) => {
                    ...
                    const isOverdueA = dateA < todayStr && a.Status !== "Done";
                    const isCompletedA = a.Status === "Done";
                    const isTodayA = dateA === todayStr && a.Status !== "Done";

                    const isOverdueB = dateB < todayStr && b.Status !== "Done";
                    const isCompletedB = b.Status === "Done";
                    const isTodayB = dateB === todayStr && b.Status !== "Done";
                        ...
                        <div className="space-y-2 max-h-[185px] overflow-y-auto pr-1">
                          <AnimatePresence>
                            {sortedAgenda.map((s) => {
                              ...
                              const isOverdue = schedulerDateStr < todayStr && s.Status !== "Done";
                              const isCompleted = s.Status === "Done";
                              const isToday = schedulerDateStr === todayStr && s.Status !== "Done";
                                ...
                                >
                                  <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center gap-2 mt-0.5">
                                      ...
                                      {isCompleted && <span className="text-[10px] text-emerald-500 dark:text-emerald-400 font-bold">✓ Done</span>}
                                    </div>
                                  </div>
        ...
        {/* KOLOM KANAN: Riwayat Log Aktivitas (FULL MURNI DATABASE) */}
        <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-md border border-gray-100 dark:border-slate-700/60 flex flex-col h-[660px] transition-colors duration-200">
          ...
          <div className="overflow-x-auto overflow-y-auto flex-1 pr-1 pb-2 scrollbar-thin dark:scrollbar-thumb-slate-700">
            <table className="w-full text-left border-collapse text-xs table-fixed min-w-[450px]">
              ...
              <tbody>
                {dbLogs.length === 0 ? (
                  <tr><td colSpan="3" className="text-center py-6 text-gray-400 dark:text-slate-400">Tidak ada riwayat log di database.</td></tr>
                ) : (
                  dbLogs.map((log) => (
                    <tr
                      key={log.id}
                      className={`border-b border-gray-100 dark:border-slate-700/50 hover:bg-gray-50 dark:hover:bg-slate-700/30 transition-colors ${
                        log.isFromScheduler ? "bg-emerald-50/10 dark:bg-emerald-950/10" : ""
                      }`}
                    >
                      <td className="py-3 pr-4 font-medium text-gray-700 dark:text-slate-200 truncate whitespace-nowrap">
                        <span className="inline-flex items-center gap-1.5">
                          {log.isFromScheduler && (
                            <span className="text-[9px] bg-emerald-100 dark:bg-emerald-900/50 text-emerald-700 dark:text-emerald-300 px-1.5 py-0.5 rounded font-bold">
                              Agenda
                            </span>
                          )}
                          {log.jenis}
                      ...
                      <td className={`py-3 font-semibold whitespace-nowrap ${
                        log.isFromScheduler ? "text-emerald-600 dark:text-emerald-400" : "text-gray-600 dark:text-slate-300"
                      }`}>
                        {log.keterangan}
                      </td>
                    </tr>
